...
// PrepareInterview Preparation Kits1 Month Preparation KitWeek 1Mini-Max Sum
...
function miniMaxSum(arr) {
  ...
  arr.sort((a, b) => a - b);
  const max = arr.reduce((acc, elem, i) => (i > 0 ? acc + elem : acc), 0);
  const min = arr.reduce(
    (acc, elem, i) => (i < arr.length - 1 ? acc + elem : acc),
    0
  );
  console.log(min, max);
}
